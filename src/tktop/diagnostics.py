...
async def run_doctor(settings_path: Path | None = None) -> DoctorReport:
    ...
    if resolved_settings_path.exists():
        checks.append(
            DiagnosticCheck("Config", "ok", f"settings file: {resolved_settings_path}")
        )
    else:
        checks.append(
            DiagnosticCheck(
...
async def _check_adapter(checks: list[DiagnosticCheck], cfg: Config) -> None:
    ...
    try:
        ...
        checks.append(DiagnosticCheck("Adapter", "error", str(exc)))
        return
    ...
    if choice == "codex" and not Path(cfg.codex_dir).is_dir():
        checks.append(
            DiagnosticCheck("Adapter", "error", f"codex adapter data dir missing: {cfg.codex_dir}")
        )
        return
    ...
    try:
        ...
        checks.append(DiagnosticCheck("Adapter", "error", f"discovery failed: {exc}"))
        return
...
def _check_provider(checks: list[DiagnosticCheck], cfg: Config) -> None:
    ...
    if provider == "ollama":
        if cfg.ollama_host:
            checks.append(DiagnosticCheck("LLM provider", "ok", f"ollama host: {cfg.ollama_host}"))
        else:
            checks.append(DiagnosticCheck("LLM provider", "warn", "ollama host is not configured"))
        return
    ...
    if provider == "anthropic":
        ...
        checks.append(DiagnosticCheck("LLM provider", status, message))
        return
    ...
    if provider == "vertex":
        if cfg.vertex_project and cfg.vertex_region:
            ...
            checks.append(
                DiagnosticCheck("LLM provider", "warn", "vertex project or region missing")
            )
        return
    ...
    if provider == "openai":
        if cfg.openai_base_url and cfg.openai_api_key:
            ...
            checks.append(DiagnosticCheck("LLM provider", "warn", "openai API key missing"))
        else:
            checks.append(DiagnosticCheck("LLM provider", "warn", "openai base URL missing"))
        return

    checks.append(DiagnosticCheck("LLM provider", "error", f"unknown provider: {cfg.llm_provider}"))
